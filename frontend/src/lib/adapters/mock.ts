...
export function createMockAdapter(overrides: Partial<BackendAdapter> = {}): BackendAdapter {
  const base: BackendAdapter = {
    ...
    InspectWorkspaceGitServe: async (): Promise<GitServeReport> => ({
      path: '', git_available: true, state: '', is_repo: false, has_commits: false,
      uncommitted_paths: 0, has_gitignore: false, files: 0, bytes: 0, truncated: false,
    }),
    EnableWorkspaceGitServe: async (): Promise<Workspace> => mockWorkspace(),
    DisableWorkspaceGitServe: async (): Promise<Workspace> => mockWorkspace(),
    GetWorkspaceCheckout: async (workspaceID: string): Promise<WorkspaceCheckoutInfo> => ({
      workspace_id: workspaceID, has_copy: false, status: 'idle',
      dirty: false, ahead: 0, behind: 0, diverged: false, git_available: false,
    }),
    MaterializeWorkspace: async () => {},
    PullWorkspaceCheckout: async (): Promise<WorkspaceSyncResult> => ({ status: 'up_to_date' }),
    PushWorkspaceCheckout: async (): Promise<WorkspaceSyncResult> => ({ status: 'up_to_date' }),
    MergeWorkspaceCheckout: async (): Promise<WorkspaceSyncResult> => ({ status: 'ok' }),
    ForgetWorkspaceCheckout: async () => {},
    ...
    ToggleChatBookmark: async () => emptyChat(),
    ToggleProjectChatBookmark: async () => emptyChat(),
