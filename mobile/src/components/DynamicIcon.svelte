<script lang="ts">
  // Mobile DynamicIcon — same shape and behaviour as the desktop's
  // DynamicIcon.svelte (which is the source of truth). Kept thin so
  // bug fixes can be ported one-for-one. Uses the shared icon map +
  // string parser so both apps render the same icon strings the same
  // way.
