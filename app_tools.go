...
//
//   - executeToolCall: the edit-mode card-chat executor. Mutates the
...
//   - executeProjectToolCall: the project-chat executor. Operates on a
//     projectChatScope (see app_chat.go) and refuses to touch cards
//     outside scope to defend against LLM hallucinations.
//
// Plus two staging paths for suggest mode (stageToolCall,
// stageProjectToolCall) that convert a tool call into PendingEdits on
...
type cardToolHandler func(a *App, cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion)
...
// startup, not per call. Method values on *App are first-class
// functions that accept the receiver as their first argument, so
// the closure cost is zero.
//
// Follow-up candidates: executeProjectToolCall, stageToolCall, and
// stageProjectToolCall all still use switches and should be migrated
...
var cardToolHandlers = map[string]cardToolHandler{
	"set_title":       (*App).toolSetTitle,
	"set_due_date":    (*App).toolSetDueDate,
	"set_card_type":   (*App).toolSetCardType,
	"set_fields":      (*App).toolSetFields,
	"update_blocks":   (*App).toolSetFields, // alias — same handler
	"add_tags":        (*App).toolAddTags,
	"add_field":       (*App).toolAddField,
	"suggest_pin":     (*App).toolSuggestPin,
	"configure_agent": (*App).toolConfigureAgent,
	"web_fetch":       (*App).toolWebFetch,
	"web_search":      (*App).toolWebSearch,
}
...
func (a *App) executeToolCall(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
	if handler, ok := cardToolHandlers[tc.Name]; ok {
		return handler(a, cardID, card, tc, allCats)
	}
	return "error: unknown tool " + tc.Name, nil, nil
}

func (a *App) toolSetTitle(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolSetDueDate(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolSetCardType(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolSetFields(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolAddTags(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolAddField(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolSuggestPin(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolConfigureAgent(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolWebFetch(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
...
func (a *App) toolWebSearch(cardID string, card *model.Card, tc llm.ToolCall, allCats []CategoryPath) (string, *model.ToolAction, *model.PinSuggestion) {
