...
document.addEventListener('click', (e) => {
  ...
  if (bruv) {
    ...
    if (bruv.startsWith('card:')) {
      ...
      const path = bruv.slice(8)
      const [brand, stream, project] = path.split('/')
      if (brand && stream && project) {
        document.dispatchEvent(new CustomEvent('bruv:navigate', { detail: { type: 'project', brand, stream, project } }))
      }
    }
    return
  }
